...
- `AgentComposer` and its composer-only helpers carry an `@deprecated` tag.
  ...
  canonical composer. It is a superset: it adds the attachments lifecycle,
  send-failure recovery, seeds, quiet and labeled controls, floating
  elevation, and the circular icon send through `sendVariant="icon"`.
...
- **`--text-dim` now clears WCAG AA against the surface ladder this package actually ships.** `@tangle-network/ui` 11.2.4 moved the Input/Textarea hint, the StatCard subtitle and the TerminalLine timestamp off alpha-faded foregrounds (`text-muted-foreground/70`, `opacity-70`, `opacity-50`) and onto a solid `text-[var(--text-dim)]`. That change is right — a translucent foreground renders as its colour composited over whatever plane sits behind it, so one class measured a different ratio on a card than on the canvas. But the token's *value* was tuned against brand 1.3.0's newer, lighter ladder, and this package resolves brand 1.1.0's darker one, so the correct class landed the wrong colour. Measured in Chromium against the built stylesheet, dark `#6b6b73` read 3.73:1 on the canvas, 3.31:1 on a card and 2.65:1 on the L4 overlay; light `#8e8e98` read 2.79:1 on the canvas. The dark input hint had been 4.63:1 under the old alpha class, so this was a regression below AA in every product on the precompiled theme. The token is corrected for the ladder shipped here — dark `#6b6b73` → `#92929a`, light `#8e8e98` → `#6b6b75` — which clears 4.5:1 on all eleven planes this package paints content on, in both themes, with the worst position at 4.52:1 (dark L4) and 4.54:1 (light canvas). Brand's named themes are untouched: each declares its own ladder and `--text-dim` as a matched pair, so the override is guarded with `:not([data-theme])`.

- **The precompiled bundle now emits the brand text ramp unconditionally.** `dist/globals.css` emitted *no* rule for `text-[var(--text-dim)]`. The `@source` glob scans the `@tangle-network/ui` installed **here at build time**, and a consumer resolves its own — the peer range permits a newer one — so every utility that newer version introduced is missing from the bundle. `@tangle-network/ui` 11.2.4 introduced exactly that: apps running their own Tailwind/UnoCSS scan over `@tangle-network/ui` were fine, while apps on the precompiled bundle (physim, blueprint-agent) rendered hints, subtitles and timestamps at inherited body colour — 15–17:1, identical to body copy. Not a contrast failure; the tier is simply gone, which no contrast check can see. A scan cannot fix this by construction, so the four text tiers (`--text-primary`, `--text-secondary`, `--text-muted`, `--text-dim`) are now declared with `@source inline(...)` and asserted from the built bytes. The fix holds for **any** `@tangle-network/ui` in the peer range, not just the one this package happens to resolve.

- **The `@source` globs are now the whole of what the bundle compiles from (`@import "tailwindcss" source(none)`).** Tailwind's automatic content detection walks the project root for anything not gitignored, so the bundle also absorbed class-shaped strings from `scripts/`, `docs/`, `.storybook/` and `CHANGELOG.md`. That was dead weight, and it made the new text-ramp check unfalsifiable: the check lists the utilities it verifies as literal strings, detection scanned that file, and the utilities were emitted *by* the check — every assertion passed while measuring nothing. Turning detection off drops 12 rules, all verified unused by any shipped source (`.text-[var(--x)]` and friends from test fixtures, `.accent-foreground` matched out of `text-accent-foreground`, `.pb-12`/`.pt-14` from prose); no rule used by a shipped component is lost.

- **The build's forwarded-utility check derives its requirement instead of hard-coding it.** It previously compared the built CSS against a hand-maintained list of six marker strings; nobody added the new token utilities to that list, which is why this shipped. It now reads the same `src/` tree the `@source` globs scan and requires every `*-[var(--token)]` utility found there, naming the utility *and its source file* when one is missing. It also fails, rather than passing quietly, when that source cannot be read at all, and when the bundle was compiled against a `@tangle-network/ui` older than the peer floor this package declares.

### Tooling

- `scripts/text-dim-contrast.mjs` measures `--text-dim` against every plane in both themes in real Chromium, resolving each surface through its real `bg-*` utility and rasterising the result so `color-mix()` and alpha compositing are answered by the renderer rather than by arithmetic. `scripts/text-dim-contrast.test.mjs` re-derives the same numbers from the token files with no browser and runs on every commit; the browser script asserts the two agree.
...
- **A run's graph opens as cards, and a definition's opens as tiles.** The density default now follows whether there is a run to read, rather than always starting collapsed. A definition is structure — the tiles are what make its shape legible, and there are no results to show — while a run is opened for its results, and starting it as tiles put every answer behind a toggle the reader had to find first. Passing `defaultCompact` still pins the density either way, and the toggle is unchanged.
  **Behavioural change for callers that omit `defaultCompact`:** the prop's default was `true` and is now `nodeState === undefined`, so a `WorkflowGraph` given run state opens expanded where it previously opened collapsed. A caller that wants the old behaviour passes `defaultCompact` explicitly; one that never passes it and never passes `nodeState` is unaffected.
...
- **An agent still says what it is going to do before it has done it.** Trading the prompt for the answer is only right once there is an answer: a queued fan-out branch that showed neither rendered as an anonymous card, and three branches of one model produced three identical ones. The body now falls back to the prompt while the node is still going to produce something — and a node that has FINISHED without producing anything says so, rather than showing its prompt where its result belongs. The card's strip names the agent as well as the model for the same reason.
- **A parked decision no longer truncates the question it is asking.** "Waiting on you" is the widest status pill there is, and on one row beside the type mark it left about 120px of a 292px card for the title — so the one node the reader has to act on cut off both its question and its options. The decision's header stacks instead: the question takes the full width, and the options share the line below it with the pill.
- **A node the host sent no run state for renders as part of the run.** The layout reserves the run bands for every action the moment a graph has any run state, so a node with no entry of its own — one the run has not reached — was drawing its definition card into a run-sized box and leaving over a hundred pixels of blank beneath it. It now renders as the not-yet-reached step it is, which is how the edge pointing at it was already drawn.
- **The output well fills the space its kind reserved** rather than hugging a one-line body and leaving an orphan gap above the footer, and control flow's card centres what little it has to say. A reservation sized for the worst case cannot shrink to the actual one, so the space is spent rather than left looking unfinished.
