...
- **New `./connectors` module** — host-agnostic building blocks for a browsable integration/connector library, so any app (not just the platform) can render the same catalog over its own hub data. Three presentational components plus a helper, all data-in/props-down with the host owning fetching, navigation, and connect flows:
