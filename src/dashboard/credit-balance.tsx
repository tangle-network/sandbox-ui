...
export function CreditBalance({
  ...
  return (
    <div className={cn("bg-card p-8 rounded-xl flex flex-col justify-between border border-border", className)}>
      <div>
        <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-widest mb-2">Available Credits</h3>
        <div className="text-5xl font-extrabold text-primary tracking-tighter mb-3">${amount.toFixed(2)}</div>
        <p className="text-sm text-muted-foreground leading-relaxed">{description}</p>
      </div>
      {onTopUp && (
        <div className="space-y-3 mt-8">
          <div className="bg-background border border-border p-1 rounded-lg flex items-center">
            <input
              type="text"
              value={`$${topUpValue}`}
