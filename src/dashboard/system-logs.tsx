  ...
  useEffect(() => {
    ...
    async function fetchLogs() {
      try {
        ...
        if (isMounted) setError(err instanceof Error ? err.message : "Failed to fetch logs");
      }
    ...
    <div className={cn("glass-panel flex flex-col h-full bg-[#0a0a0c] text-[var(--code-keyword)] font-mono text-sm leading-relaxed overflow-hidden", className)}>
      <div className="flex-none flex items-center justify-between border-b border-glass-border bg-background/80 backdrop-blur-md px-4 py-2">
        ...
        <div className="flex items-center gap-3">
          {error && <span className="text-red-400 text-xs flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 animate-ping"></span> Error fetching logs</span>}
          <button 
            onClick={() => {
         ...
         {logs.length === 0 && !error ? (
           ...
           logs.map((log, i) => (
             <div key={i} className="break-words">
                <span className="text-white/40 mr-3 select-none">[{log.timestamp || i.toString().padStart(4, '0')}]</span>
                <span className="text-accent/70 mr-2">[{log.level}]</span>
                <span className="text-white/60 mr-2">[{log.scope}]</span>
                <span className={log.level.toUpperCase() === "ERROR" || log.message.toLowerCase().includes("failed") ? "text-red-400" : log.level.toUpperCase() === "WARN" ? "text-yellow-400" : "text-white/80"}>
                  {log.message}
                </span>
             </div>
           ))
